...
HeapprofdConfig::HeapprofdConfig(const HeapprofdConfig& from)
  ...
  ::memcpy(&sampling_interval_bytes_, &from.sampling_interval_bytes_,
    static_cast<size_t>(reinterpret_cast<char*>(&max_heapprofd_memory_kb_) -
    reinterpret_cast<char*>(&sampling_interval_bytes_)) + sizeof(max_heapprofd_memory_kb_));
...
void HeapprofdConfig::SharedCtor() {
  ...
  ::memset(&continuous_dump_config_, 0, static_cast<size_t>(
      reinterpret_cast<char*>(&max_heapprofd_memory_kb_) -
      reinterpret_cast<char*>(&continuous_dump_config_)) + sizeof(max_heapprofd_memory_kb_));
}
...
// @@protoc_insertion_point(message_clear_start:perfetto.protos.HeapprofdConfig)
  ...
  if (cached_has_bits & 0x00007f00u) {
    ::memset(&min_anonymous_memory_kb_, 0, static_cast<size_t>(
        reinterpret_cast<char*>(&max_heapprofd_memory_kb_) -
        reinterpret_cast<char*>(&min_anonymous_memory_kb_)) + sizeof(max_heapprofd_memory_kb_));
  }
  _has_bits_.Clear();
  _internal_metadata_.Clear();
}
