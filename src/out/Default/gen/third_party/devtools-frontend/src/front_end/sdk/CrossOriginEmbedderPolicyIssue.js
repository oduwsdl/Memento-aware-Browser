...
function CorpNotSameOriginAfterDefaultedToSameOriginByCoepMessage() {
    ...
    message.createChild('p').textContent = ls `Because your site has the Cross-Origin Embedder Policy (COEP) enabled, each resource must specify a suitable Cross-Origin Resource Policy (CORP).
...
function CoepFrameResourceNeedsCoepHeaderMessage() {
    const message = document.createElement('div');
    message.classList.add('message');
    message.createChild('p').textContent = ls `Because your site has the Cross-Origin Embedder Policy (COEP) enabled, each embedded iframe must also specify this policy.
  This behavior protects private data from being exposed to untrusted third party sites.`;
    ...
    solvedBy.createChild('span').textContent = ls `To solve this, add the following to the embedded frame’s HTML response header:` +
        ' ';
...
function CorpNotSameSiteMessage() {
    const message = document.createElement('div');
    message.classList.add('message');
    message.createChild('p').textContent = ls `Your site tries to access an external resource that only allows same-site usage.
...
function CorpNotSameOriginMessage() {
    const message = document.createElement('div');
    message.classList.add('message');
    message.createChild('p').textContent = ls `Your site tries to access an external resource that only allows same-origin usage.
...
const issueDescriptions = new Map([
    ...
    ['CrossOriginEmbedderPolicy::CoopSandboxedIframeCannotNavigateToCoopPage', {
            ...
            message: () => textOnlyMessage(ls `A document with a Cross-Origin Opener Policy (COOP) was blocked from loading in an iframe, because the iframe specifies a sandbox attribute.
