...
export function drawGridNumbers(config, bounds) {
    const showPositiveColumns = config.gridHighlightConfig.showPositiveLineNumbers && config.positiveColumnLineNumberOffsets;
    const showPositiveRows = config.gridHighlightConfig.showPositiveLineNumbers && config.positiveRowLineNumberOffsets;
    const showNegativeColumns = config.gridHighlightConfig.showNegativeLineNumbers && config.negativeColumnLineNumberOffsets;
    ...
    if (showPositiveColumns) {
        for (const [i, offset] of config.positiveColumnLineNumberOffsets.entries()) {
            ...
            _placeColumnLabel(labelContainer, (i + 1).toString(), bounds.minX + offset, bounds.minY, isFirstColumn, isLastColumn);
        }
    }
    if (showPositiveRows) {
        for (const [i, offset] of config.positiveRowLineNumberOffsets.entries()) {
            ...
            _placeRowLabel(labelContainer, (i + 1).toString(), bounds.minX, bounds.minY + offset, isTopRow, isBottomRow, avoidColumnLabel);
        }
    }
    if (showNegativeColumns) {
        for (const [i, offset] of config.negativeColumnLineNumberOffsets.entries()) {
            ...
            _placeColumnLabel(labelContainer, index.toString(), bounds.minX + offset, bounds.maxY, isFirstColumn, isLastColumn, 
            /* isNegative */ true);
        }
    }
    if (showNegativeRows) {
        for (const [i, offset] of config.negativeRowLineNumberOffsets.entries()) {
            ...
            const avoidColumnLabel = (isTopRow && showPositiveColumns) || (isBottomRow && showNegativeColumns);
            _placeRowLabel(labelContainer, index.toString(), bounds.maxX, bounds.minY + offset, isTopRow, isBottomRow, avoidColumnLabel, 
            /* isNegative */ true);
        }
    }
}
