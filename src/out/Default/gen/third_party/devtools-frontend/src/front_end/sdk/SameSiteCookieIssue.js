...
const sameSiteUnspecifiedErrorRead = {
    ...
    message: () => textMessageWithResolutions(ls `Because a cookie's |SameSite| attribute was not set or is invalid, it defaults to |SameSite=Lax|, which prevents the cookie from being sent in a cross-site request.
       This behavior protects user data from accidentally leaking to third parties and cross-site request forgery.`, resolveBySentence, resolutionsRead),
...
const sameSiteUnspecifiedErrorSet = {
    title: ls `Indicate whether a cookie is intended to be set in a cross-site context by specifying its SameSite attribute`,
    message: () => textMessageWithResolutions(ls `Because a cookie's |SameSite| attribute  was not set or is invalid, it defaults to |SameSite=Lax|, which prevents the cookie from being set in a cross-site context.
       This behavior protects user data from accidentally leaking to third parties and cross-site request forgery.`, resolveBySentence, resolutionsSet),
    issueKind: IssueKind.BreakingChange,
    links: [{ link: ls `https://web.dev/samesite-cookies-explained/`, linkTitle: ls `SameSite cookies explained` }],
};
const sameSiteUnspecifiedWarnRead = {
    title: ls `Indicate whether to send a cookie in a cross-site request by specifying its SameSite attribute`,
    message: () => textMessageWithResolutions(ls `Because a cookie's |SameSite| attribute was not set or is invalid, it defaults to |SameSite=Lax|, which will prevent the cookie from being sent in a cross-site request in a future version of the browser.
       This behavior protects user data from accidentally leaking to third parties and cross-site request forgery.`, resolveBySentence, resolutionsRead),
    issueKind: IssueKind.BreakingChange,
    links: [{ link: ls `https://web.dev/samesite-cookies-explained/`, linkTitle: ls `SameSite cookies explained` }],
};
const sameSiteUnspecifiedWarnSet = {
    ...
    message: () => textMessageWithResolutions(ls `Because a cookie's |SameSite| attribute was not set or is invalid, it defaults to |SameSite=Lax|, which will prevents the cookie from being set in a cross-site context in a future version of the browser.
       This behavior protects user data from accidentally leaking to third parties and cross-site request forgery.`, resolveBySentence, resolutionsSet),
    issueKind: IssueKind.BreakingChange,
    links: [{ link: ls `https://web.dev/samesite-cookies-explained/`, linkTitle: ls `SameSite cookies explained` }],
};
const sameSiteNoneInsecureErrorRead = {
    ...
    message: () => textMessageWithResolutions(ls `Cookies marked with |SameSite=None| must also be marked with |Secure| to get sent in cross-site requests.
       This behavior protects user data from being sent over an insecure connection.`, resolveBySentence, resolutionsRead),
    issueKind: IssueKind.BreakingChange,
    links: [{ link: ls `https://web.dev/samesite-cookies-explained/`, linkTitle: ls `SameSite cookies explained` }],
};
const sameSiteNoneInsecureErrorSet = {
    ...
    message: () => textMessageWithResolutions(ls `Cookies marked with |SameSite=None| must also be marked with |Secure| to allow setting them in a cross-site context.
       This behavior protects user data from being sent over an insecure connection.`, resolveBySentence, resolutionsSet),
    issueKind: IssueKind.BreakingChange,
    links: [{ link: ls `https://web.dev/samesite-cookies-explained/`, linkTitle: ls `SameSite cookies explained` }],
};
const sameSiteNoneInsecureWarnRead = {
    title: ls `Mark cross-site cookies as Secure to allow them to be sent in cross-site requests`,
    message: () => textMessageWithResolutions(ls `In a future version of the browser, cookies marked with |SameSite=None| must also be marked with |Secure| to get sent in cross-site requests.
       This behavior protects user data from being sent over an insecure connection.`, resolveBySentence, resolutionsRead),
    issueKind: IssueKind.BreakingChange,
    links: [{ link: ls `https://web.dev/samesite-cookies-explained/`, linkTitle: ls `SameSite cookies explained` }],
};
const sameSiteNoneInsecureWarnSet = {
    title: ls `Mark cross-site cookies as Secure to allow setting them in cross-site contexts`,
    message: () => textMessageWithResolutions(ls `In a future version of the browser, cookies marked with |SameSite=None| must also be marked with |Secure| to allow setting them in a cross-site context.
       This behavior protects user data from being sent over an insecure connection.`, resolveBySentence, resolutionsSet),
    issueKind: IssueKind.BreakingChange,
    links: [{ link: ls `https://web.dev/samesite-cookies-explained/`, linkTitle: ls `SameSite cookies explained` }],
};
...
const resolveBySentenceForDowngrade = ls `Resolve this issue by migrating your site entirely to HTTPS. It is also recommended to mark the cookie with the |Secure| attribute if that is not already the case.`;
/**
...
function sameSiteWarnStrictLaxDowngradeStrict(isSecure) {
    ...
    return {
        ...
        message: () => textMessageWithResolutions(ls `A cookie is being sent from ${origin} context to ${destination} origin on a navigation and is specified with |SameSite=Strict|.
        Because this cookie is being sent across schemes on the same site, it will not be sent in a future version of Chrome.
        This behavior protects user data from request forgery by network attackers.`, resolveBySentenceForDowngrade, []),
        issueKind: IssueKind.BreakingChange,
...
function sameSiteWarnCrossDowngradeRead(isSecure) {
    ...
    return {
        ...
        message: () => textMessageWithResolutions(ls `A cookie is being sent from ${origin} context to ${destination} origin and is specified with |SameSite=Strict| or |SameSite=Lax|.
        Because this cookie is being sent across schemes on the same site, it will not be sent in a future version of Chrome.
        This behavior protects user data from request forgery by network attackers.`, resolveBySentenceForDowngrade, []),
        issueKind: IssueKind.BreakingChange,
        links: schemefulSameSiteArticles,
...
function sameSiteWarnCrossDowngradeSet(isSecure) {
    ...
    return {
        ...
        message: () => textMessageWithResolutions(ls `A cookie is being set by ${origin} origin in ${destination} context and is specified with |SameSite=Strict| or |SameSite=Lax|.
