...
class MODULES_EXPORT GamepadEventInit : public EventInit {
 public:
  ...
  bool hasGamepadNonNull() const { return has_gamepad_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasGamepadNonNull| in advance to check the condition.
  Gamepad* gamepadNonNull() const {
    DCHECK(hasGamepadNonNull());
    return gamepad_;
  }
