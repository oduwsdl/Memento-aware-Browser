...
class MODULES_EXPORT NotificationAction : public IDLDictionaryBase {
 public:
  ...
  bool hasPlaceholderNonNull() const { return !placeholder_.IsNull(); }
  // Returns the value if this member has a non-null value.  Call
  // |hasPlaceholderNonNull| in advance to check the condition.
  const String& placeholderNonNull() const {
    DCHECK(hasPlaceholderNonNull());
    return placeholder_;
  }
