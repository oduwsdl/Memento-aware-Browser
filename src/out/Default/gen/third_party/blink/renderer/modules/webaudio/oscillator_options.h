...
class MODULES_EXPORT OscillatorOptions : public AudioNodeOptions {
 public:
  ...
  bool hasPeriodicWaveNonNull() const { return has_periodic_wave_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasPeriodicWaveNonNull| in advance to check the condition.
  PeriodicWave* periodicWaveNonNull() const {
    DCHECK(hasPeriodicWaveNonNull());
    return periodic_wave_;
  }
