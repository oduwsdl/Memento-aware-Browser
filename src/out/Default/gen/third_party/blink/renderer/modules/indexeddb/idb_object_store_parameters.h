...
class MODULES_EXPORT IDBObjectStoreParameters : public IDLDictionaryBase {
 public:
  ...
  bool hasKeyPathNonNull() const { return !key_path_.IsNull(); }
  // Returns the value if this member has a non-null value.  Call
  // |hasKeyPathNonNull| in advance to check the condition.
  const StringOrStringSequence& keyPathNonNull() const {
    DCHECK(hasKeyPathNonNull());
    return key_path_;
  }
