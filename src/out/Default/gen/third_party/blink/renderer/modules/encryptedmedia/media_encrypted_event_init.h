...
class MODULES_EXPORT MediaEncryptedEventInit : public EventInit {
 public:
  ...
  bool hasInitDataNonNull() const { return has_init_data_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasInitDataNonNull| in advance to check the condition.
  DOMArrayBuffer* initDataNonNull() const {
    DCHECK(hasInitDataNonNull());
    return init_data_;
  }
