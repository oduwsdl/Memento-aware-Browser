...
class MODULES_EXPORT PaymentMethodChangeEventInit : public PaymentRequestUpdateEventInit {
 public:
  ...
  static PaymentMethodChangeEventInit* Create(v8::Isolate* isolate) {
    return MakeGarbageCollected<PaymentMethodChangeEventInit>();
  }
  ...
  bool hasMethodDetailsNonNull() const { return !(method_details_.IsEmpty() || method_details_.IsNull() || method_details_.IsUndefined()); }
  // Returns the value if this member has a non-null value.  Call
  // |hasMethodDetailsNonNull| in advance to check the condition.
  ScriptValue methodDetailsNonNull() const {
    DCHECK(hasMethodDetailsNonNull());
    return method_details_;
  }
