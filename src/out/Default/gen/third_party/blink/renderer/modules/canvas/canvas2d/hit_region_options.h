...
class MODULES_EXPORT HitRegionOptions : public IDLDictionaryBase {
 public:
  ...
  static HitRegionOptions* Create(v8::Isolate* isolate) {
    return MakeGarbageCollected<HitRegionOptions>();
  }
  ...
  bool hasControlNonNull() const { return has_control_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasControlNonNull| in advance to check the condition.
  Element* controlNonNull() const {
    DCHECK(hasControlNonNull());
    return control_;
  }
  ...
  // Returns true iff this member has a non-null value.  Returns false if the
  // value is missing or a null value.
  bool hasPathNonNull() const { return has_path_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasPathNonNull| in advance to check the condition.
  Path2D* pathNonNull() const {
    DCHECK(hasPathNonNull());
    return path_;
  }
