...
class MODULES_EXPORT MediaStreamEventInit : public EventInit {
 public:
  ...
  bool hasStreamNonNull() const { return has_stream_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasStreamNonNull| in advance to check the condition.
  MediaStream* streamNonNull() const {
    DCHECK(hasStreamNonNull());
    return stream_;
  }
