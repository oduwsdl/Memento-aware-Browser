...
class MODULES_EXPORT MediaCapabilitiesDecodingInfo : public MediaCapabilitiesInfo {
 public:
  ...
  bool hasKeySystemAccessNonNull() const { return has_key_system_access_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasKeySystemAccessNonNull| in advance to check the condition.
  MediaKeySystemAccess* keySystemAccessNonNull() const {
    DCHECK(hasKeySystemAccessNonNull());
    return key_system_access_;
  }
