...
class MODULES_EXPORT AudioBufferSourceOptions : public IDLDictionaryBase {
 public:
  ...
  bool hasBufferNonNull() const { return has_buffer_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasBufferNonNull| in advance to check the condition.
  AudioBuffer* bufferNonNull() const {
    DCHECK(hasBufferNonNull());
    return buffer_;
  }
