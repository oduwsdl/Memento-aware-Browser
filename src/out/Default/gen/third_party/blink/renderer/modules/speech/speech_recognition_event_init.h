...
class MODULES_EXPORT SpeechRecognitionEventInit : public EventInit {
 public:
  ...
  bool hasResultsNonNull() const { return has_results_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasResultsNonNull| in advance to check the condition.
  SpeechRecognitionResultList* resultsNonNull() const {
    DCHECK(hasResultsNonNull());
    return results_;
  }
