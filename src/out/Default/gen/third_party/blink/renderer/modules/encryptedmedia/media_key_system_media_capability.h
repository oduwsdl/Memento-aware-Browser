...
class MODULES_EXPORT MediaKeySystemMediaCapability : public IDLDictionaryBase {
 public:
  ...
  bool hasEncryptionSchemeNonNull() const { return !encryption_scheme_.IsNull(); }
  // Returns the value if this member has a non-null value.  Call
  // |hasEncryptionSchemeNonNull| in advance to check the condition.
  const String& encryptionSchemeNonNull() const {
    DCHECK(hasEncryptionSchemeNonNull());
    return encryption_scheme_;
  }
