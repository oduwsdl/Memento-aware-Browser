...
// clang-format off
#ifndef THIRD_PARTY_BLINK_RENDERER_MODULES_COOKIE_STORE_COOKIE_INIT_H_
#define THIRD_PARTY_BLINK_RENDERER_MODULES_COOKIE_STORE_COOKIE_INIT_H_
...
class MODULES_EXPORT CookieInit : public IDLDictionaryBase {
 public:
  ...
  bool hasDomainNonNull() const { return !domain_.IsNull(); }
  // Returns the value if this member has a non-null value.  Call
  // |hasDomainNonNull| in advance to check the condition.
  const String& domainNonNull() const {
    DCHECK(hasDomainNonNull());
    return domain_;
  }
  ...
  // Returns true iff this member has a non-null value.  Returns false if the
  // value is missing or a null value.
  bool hasExpiresNonNull() const { return has_expires_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasExpiresNonNull| in advance to check the condition.
...
void CookieInit::setDomainToNull() {
  domain_ = String();
}

void CookieInit::setExpires(uint64_t value) {
  expires_ = value;
  has_expires_ = true;
}

void CookieInit::setExpiresToNull() {
  has_expires_ = false;
}
...
void CookieInit::setSameSite(const String& value) {
  same_site_ = value;
}
...
#endif  // THIRD_PARTY_BLINK_RENDERER_MODULES_COOKIE_STORE_COOKIE_INIT_H_
