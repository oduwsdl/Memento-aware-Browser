...
class CORE_EXPORT AnimationPlaybackEventInit : public EventInit {
 public:
  ...
  bool hasCurrentTimeNonNull() const { return has_current_time_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasCurrentTimeNonNull| in advance to check the condition.
  double currentTimeNonNull() const {
    DCHECK(hasCurrentTimeNonNull());
    return current_time_;
  }
  ...
  // Returns true iff this member has a non-null value.  Returns false if the
  // value is missing or a null value.
  bool hasTimelineTimeNonNull() const { return has_timeline_time_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasTimelineTimeNonNull| in advance to check the condition.
  double timelineTimeNonNull() const {
    DCHECK(hasTimelineTimeNonNull());
    return timeline_time_;
  }
