...
void CoreProbeSink::AddInspectorIssueReporter(InspectorIssueReporter* agent) {
  ...
  inspector_issue_reporters_.insert(agent);
...
void CoreProbeSink::RemoveInspectorIssueReporter(InspectorIssueReporter* agent) {
  ...
  inspector_issue_reporters_.erase(agent);
...
void CoreProbeSink::AddInspectorAuditsAgent(InspectorAuditsAgent* agent) {
  ...
  inspector_audits_agents_.insert(agent);
...
void CoreProbeSink::RemoveInspectorAuditsAgent(InspectorAuditsAgent* agent) {
  ...
  inspector_audits_agents_.erase(agent);
...
void ApplyUserAgentMetadataOverrideImpl(CoreProbeSink* param_core_probe_sink, base::Optional<blink::UserAgentMetadata>* ua_metadata_override) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorEmulationAgents()) {
...
void DidFailLoadingImpl(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, const ResourceError& param_resource_error, const base::UnguessableToken& param_base) {
  ...
  if (probe_sink->HasInspectorIssueReporters()) {
    for (InspectorIssueReporter* agent : probe_sink->InspectorIssueReporters())
      agent->DidFailLoading(param_core_probe_sink, identifier, param_document_loader, param_resource_error, param_base);
  }
}
...
void InspectorIssueAddedImpl(CoreProbeSink* param_core_probe_sink, InspectorIssue* param_inspector_issue) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorAuditsAgents()) {
    for (InspectorAuditsAgent* agent : probe_sink->InspectorAuditsAgents())
      agent->InspectorIssueAdded(param_inspector_issue);
  }
}
