...
public:
    ...
    String getDisposition() { return m_disposition; }
    void setDisposition(const String& value) { m_disposition = value; }
    ...
    public:
        enum {
            ...
            UrlSet = 1 << 3,
            DispositionSet = 1 << 4,
            AllFieldsSet = (FrameIdSet | ReasonSet | UrlSet | DispositionSet | 0)};
        ...
        FrameRequestedNavigationNotificationBuilder<STATE | DispositionSet>& setDisposition(const String& value)
        {
            static_assert(!(STATE & DispositionSet), "property disposition should not be set yet");
            m_result->setDisposition(value);
            return castState<DispositionSet>();
        }
...
public:
    ...
    void setGuid(const String& value) { m_guid = value; }

    String getUrl() { return m_url; }
    void setUrl(const String& value) { m_url = value; }

    String getSuggestedFilename() { return m_suggestedFilename; }
    void setSuggestedFilename(const String& value) { m_suggestedFilename = value; }
    ...
    public:
        enum {
            ...
            GuidSet = 1 << 2,
            UrlSet = 1 << 3,
            SuggestedFilenameSet = 1 << 4,
            AllFieldsSet = (FrameIdSet | GuidSet | UrlSet | SuggestedFilenameSet | 0)};
        ...
        DownloadWillBeginNotificationBuilder<STATE | GuidSet>& setGuid(const String& value)
        {
            static_assert(!(STATE & GuidSet), "property guid should not be set yet");
            m_result->setGuid(value);
            return castState<GuidSet>();
        }
        ...
        DownloadWillBeginNotificationBuilder<STATE | SuggestedFilenameSet>& setSuggestedFilename(const String& value)
        {
            static_assert(!(STATE & SuggestedFilenameSet), "property suggestedFilename should not be set yet");
            m_result->setSuggestedFilename(value);
            return castState<SuggestedFilenameSet>();
        }
...
class CORE_EXPORT DownloadProgressNotification : public Serializable{
    PROTOCOL_DISALLOW_COPY(DownloadProgressNotification);
public:
    static std::unique_ptr<DownloadProgressNotification> fromValue(protocol::Value* value, ErrorSupport* errors);

    ~DownloadProgressNotification() override { }

    String getGuid() { return m_guid; }
    void setGuid(const String& value) { m_guid = value; }

    double getTotalBytes() { return m_totalBytes; }
    void setTotalBytes(double value) { m_totalBytes = value; }

    double getReceivedBytes() { return m_receivedBytes; }
    void setReceivedBytes(double value) { m_receivedBytes = value; }

    struct CORE_EXPORT StateEnum {
        static const char* InProgress;
    ...
    std::unique_ptr<protocol::DictionaryValue> toValue() const;
    void AppendSerialized(std::vector<uint8_t>* out) const override;
    std::unique_ptr<DownloadProgressNotification> clone() const;

    template<int STATE>
    class DownloadProgressNotificationBuilder {
    public:
        enum {
            NoFieldsSet = 0,
            GuidSet = 1 << 1,
            TotalBytesSet = 1 << 2,
            ReceivedBytesSet = 1 << 3,
            StateSet = 1 << 4,
            AllFieldsSet = (GuidSet | TotalBytesSet | ReceivedBytesSet | StateSet | 0)};


        DownloadProgressNotificationBuilder<STATE | GuidSet>& setGuid(const String& value)
        {
            static_assert(!(STATE & GuidSet), "property guid should not be set yet");
            m_result->setGuid(value);
            return castState<GuidSet>();
        ...
        DownloadProgressNotificationBuilder<STATE | TotalBytesSet>& setTotalBytes(double value)
        {
            static_assert(!(STATE & TotalBytesSet), "property totalBytes should not be set yet");
            m_result->setTotalBytes(value);
            return castState<TotalBytesSet>();
        }

        DownloadProgressNotificationBuilder<STATE | ReceivedBytesSet>& setReceivedBytes(double value)
        {
            static_assert(!(STATE & ReceivedBytesSet), "property receivedBytes should not be set yet");
            m_result->setReceivedBytes(value);
            return castState<ReceivedBytesSet>();
        }

        DownloadProgressNotificationBuilder<STATE | StateSet>& setState(const String& value)
        {
        ...
        std::unique_ptr<DownloadProgressNotification> build()
        {
            static_assert(STATE == AllFieldsSet, "state should be AllFieldsSet");
            return std::move(m_result);
        }
    ...
    private:
        friend class DownloadProgressNotification;
        DownloadProgressNotificationBuilder() : m_result(new DownloadProgressNotification()) { }

        template<int STEP> DownloadProgressNotificationBuilder<STATE | STEP>& castState()
        {
            return *reinterpret_cast<DownloadProgressNotificationBuilder<STATE | STEP>*>(this);
        }

        std::unique_ptr<protocol::Page::DownloadProgressNotification> m_result;
    };

    static DownloadProgressNotificationBuilder<0> create()
    {
        return DownloadProgressNotificationBuilder<0>();
    }
