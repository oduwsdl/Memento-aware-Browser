...
CORE_EXPORT void ApplyUserAgentMetadataOverrideImpl(CoreProbeSink*, base::Optional<blink::UserAgentMetadata>*);
inline void ApplyUserAgentMetadataOverride(CoreProbeSink* param_core_probe_sink, base::Optional<blink::UserAgentMetadata>* ua_metadata_override) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorEmulationAgent))
    return;
...
inline void DidFailLoading(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, const ResourceError& param_resource_error, const base::UnguessableToken& param_base) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorTraceEvents | CoreProbeSink::kInspectorNetworkAgent | CoreProbeSink::kInspectorIssueReporter))
    return;
...
inline void DomContentLoadedEventFired(LocalFrame* param_local_frame) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kDevToolsSession | CoreProbeSink::kInspectorDOMAgent | CoreProbeSink::kInspectorPageAgent))
    return;
...
CORE_EXPORT void InspectorIssueAddedImpl(CoreProbeSink*, InspectorIssue*);
inline void InspectorIssueAdded(CoreProbeSink* param_core_probe_sink, InspectorIssue* param_inspector_issue) {
  if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorAuditsAgent))
    return;

  InspectorIssueAddedImpl(param_core_probe_sink, param_inspector_issue);
}
