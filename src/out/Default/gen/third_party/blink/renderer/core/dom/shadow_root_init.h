...
class CORE_EXPORT ShadowRootInit : public IDLDictionaryBase {
 public:
  ...
  bool hasSlotAssignment() const { return !slot_assignment_.IsNull(); }
  const String& slotAssignment() const {
    return slot_assignment_;
  }
  inline void setSlotAssignment(const String&);
...
void ShadowRootInit::setSlotAssignment(const String& value) {
  slot_assignment_ = value;
}
