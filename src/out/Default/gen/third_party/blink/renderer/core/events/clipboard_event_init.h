...
class CORE_EXPORT ClipboardEventInit : public EventInit {
 public:
  ...
  bool hasClipboardDataNonNull() const { return has_clipboard_data_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasClipboardDataNonNull| in advance to check the condition.
  DataTransfer* clipboardDataNonNull() const {
    DCHECK(hasClipboardDataNonNull());
    return clipboard_data_;
  }
