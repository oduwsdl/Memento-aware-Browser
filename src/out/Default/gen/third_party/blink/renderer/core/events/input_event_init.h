...
class CORE_EXPORT InputEventInit : public UIEventInit {
 public:
  ...
  const String& dataNonNull() const {
  ...
  // Returns true iff this member has a non-null value.  Returns false if the
  // value is missing or a null value.
  bool hasDataTransferNonNull() const { return has_data_transfer_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasDataTransferNonNull| in advance to check the condition.
  DataTransfer* dataTransferNonNull() const {
    DCHECK(hasDataTransferNonNull());
    return data_transfer_;
  }
