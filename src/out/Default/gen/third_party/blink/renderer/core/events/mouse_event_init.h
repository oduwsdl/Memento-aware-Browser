...
class CORE_EXPORT MouseEventInit : public EventModifierInit {
 public:
  ...
  bool hasRegionNonNull() const { return !region_.IsNull(); }
  // Returns the value if this member has a non-null value.  Call
  // |hasRegionNonNull| in advance to check the condition.
  const String& regionNonNull() const {
    DCHECK(hasRegionNonNull());
    return region_;
  }
  ...
  // Returns true iff this member has a non-null value.  Returns false if the
  // value is missing or a null value.
  bool hasRelatedTargetNonNull() const { return has_related_target_; }
  // Returns the value if this member has a non-null value.  Call
  // |hasRelatedTargetNonNull| in advance to check the condition.
  EventTarget* relatedTargetNonNull() const {
    DCHECK(hasRelatedTargetNonNull());
    return related_target_;
  }
