...
static void UpgradeMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!root) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("upgrade", "CustomElementRegistry", ExceptionMessages::ArgumentNotOfType(0, "Node")));
    return;
  }
