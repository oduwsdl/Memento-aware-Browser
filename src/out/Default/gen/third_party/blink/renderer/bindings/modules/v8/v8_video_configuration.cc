...
eternalV8VideoConfigurationKeys(v8::Isolate* isolate) {
...
void V8VideoConfiguration::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, VideoConfiguration* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8VideoConfigurationKeys(isolate).data();
  ...
  if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&framerate_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[5].Get(isolate)).ToLocal(&height_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (RuntimeEnabledFeatures::MediaCapabilitiesDynamicRangeEnabled()) {
    v8::Local<v8::Value> color_gamut_value;
    if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&color_gamut_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (color_gamut_value.IsEmpty() || color_gamut_value->IsUndefined()) {
      // Do nothing.
    } else {
      V8StringResource<> color_gamut_cpp_value = color_gamut_value;
      if (!color_gamut_cpp_value.Prepare(exception_state))
        return;
      ...
      if (!IsValidEnum(color_gamut_cpp_value, kValidValues, base::size(kValidValues), "ColorGamut", exception_state))
        return;
      impl->setColorGamut(color_gamut_cpp_value);
    }

    v8::Local<v8::Value> hdr_metadata_type_value;
    if (!v8Object->Get(context, keys[4].Get(isolate)).ToLocal(&hdr_metadata_type_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (hdr_metadata_type_value.IsEmpty() || hdr_metadata_type_value->IsUndefined()) {
      // Do nothing.
    } else {
      V8StringResource<> hdr_metadata_type_cpp_value = hdr_metadata_type_value;
      if (!hdr_metadata_type_cpp_value.Prepare(exception_state))
        return;
      const char* const kValidValues[] = {
          "smpteSt2086",
          "smpteSt2094-10",
      ...
      if (!IsValidEnum(hdr_metadata_type_cpp_value, kValidValues, base::size(kValidValues), "HdrMetadataType", exception_state))
        return;
      impl->setHdrMetadataType(hdr_metadata_type_cpp_value);
    }

    v8::Local<v8::Value> transfer_function_value;
    if (!v8Object->Get(context, keys[6].Get(isolate)).ToLocal(&transfer_function_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (transfer_function_value.IsEmpty() || transfer_function_value->IsUndefined()) {
      // Do nothing.
    } else {
      V8StringResource<> transfer_function_cpp_value = transfer_function_value;
      if (!transfer_function_cpp_value.Prepare(exception_state))
        return;
      const char* const kValidValues[] = {
          "srgb",
          "pq",
      ...
      if (!IsValidEnum(transfer_function_cpp_value, kValidValues, base::size(kValidValues), "TransferFunction", exception_state))
        return;
      impl->setTransferFunction(transfer_function_cpp_value);
    }
  }
}
...
bool toV8VideoConfiguration(const VideoConfiguration* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (framerate_has_value_or_default &&
      !create_property(3, framerate_value)) {
    return false;
  }
  ...
  if (height_has_value_or_default &&
      !create_property(5, height_value)) {
    return false;
  }
  ...
  if (RuntimeEnabledFeatures::MediaCapabilitiesDynamicRangeEnabled()) {
    v8::Local<v8::Value> color_gamut_value;
    bool color_gamut_has_value_or_default = false;
    if (impl->hasColorGamut()) {
      color_gamut_value = V8String(isolate, impl->colorGamut());
      color_gamut_has_value_or_default = true;
    }
    if (color_gamut_has_value_or_default &&
        !create_property(1, color_gamut_value)) {
      return false;
    }

    v8::Local<v8::Value> hdr_metadata_type_value;
    bool hdr_metadata_type_has_value_or_default = false;
    if (impl->hasHdrMetadataType()) {
      hdr_metadata_type_value = V8String(isolate, impl->hdrMetadataType());
      hdr_metadata_type_has_value_or_default = true;
    }
    if (hdr_metadata_type_has_value_or_default &&
        !create_property(4, hdr_metadata_type_value)) {
      return false;
    }

    v8::Local<v8::Value> transfer_function_value;
    bool transfer_function_has_value_or_default = false;
    if (impl->hasTransferFunction()) {
      transfer_function_value = V8String(isolate, impl->transferFunction());
      transfer_function_has_value_or_default = true;
    }
    if (transfer_function_has_value_or_default &&
        !create_property(6, transfer_function_value)) {
      return false;
    }
  }
