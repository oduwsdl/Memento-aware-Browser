...
eternalV8GPUBufferDescriptorKeys(v8::Isolate* isolate) {
...
void V8GPUBufferDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPUBufferDescriptor* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8GPUBufferDescriptorKeys(isolate).data();
  ...
  if (size_value.IsEmpty() || size_value->IsUndefined()) {
    ...
    uint64_t size_cpp_value = NativeValueTraits<IDLUnsignedLongLongEnforceRange>::NativeValue(isolate, size_value, exception_state);
    if (exception_state.HadException())
      return;
