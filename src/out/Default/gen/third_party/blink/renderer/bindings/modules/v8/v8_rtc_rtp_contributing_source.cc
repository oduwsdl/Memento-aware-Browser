...
eternalV8RTCRtpContributingSourceKeys(v8::Isolate* isolate) {
...
void V8RTCRtpContributingSource::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, RTCRtpContributingSource* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8RTCRtpContributingSourceKeys(isolate).data();
  ...
  if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&rtp_timestamp_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&source_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[4].Get(isolate)).ToLocal(&timestamp_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (RuntimeEnabledFeatures::CaptureTimeInCsrcEnabled()) {
    v8::Local<v8::Value> capture_timestamp_value;
    if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&capture_timestamp_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (capture_timestamp_value.IsEmpty() || capture_timestamp_value->IsUndefined()) {
      // Do nothing.
    } else {
      double capture_timestamp_cpp_value = NativeValueTraits<IDLDouble>::NativeValue(isolate, capture_timestamp_value, exception_state);
      if (exception_state.HadException())
        return;
      impl->setCaptureTimestamp(capture_timestamp_cpp_value);
    }
  }
}
...
bool toV8RTCRtpContributingSource(const RTCRtpContributingSource* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (rtp_timestamp_has_value_or_default &&
      !create_property(2, rtp_timestamp_value)) {
    return false;
  }
  ...
  if (source_has_value_or_default &&
      !create_property(3, source_value)) {
    return false;
  }
  ...
  if (timestamp_has_value_or_default &&
      !create_property(4, timestamp_value)) {
    return false;
  }

  if (RuntimeEnabledFeatures::CaptureTimeInCsrcEnabled()) {
    v8::Local<v8::Value> capture_timestamp_value;
    bool capture_timestamp_has_value_or_default = false;
    if (impl->hasCaptureTimestamp()) {
      capture_timestamp_value = v8::Number::New(isolate, impl->captureTimestamp());
      capture_timestamp_has_value_or_default = true;
    }
    if (capture_timestamp_has_value_or_default &&
        !create_property(1, capture_timestamp_value)) {
      return false;
    }
  }
