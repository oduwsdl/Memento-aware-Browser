...
static void SetSubDataMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  dst_offset = NativeValueTraits<IDLUnsignedLongLongEnforceRange>::NativeValue(info.GetIsolate(), info[0], exception_state);
  if (exception_state.HadException())
    return;

  ToFlexibleArrayBufferView(info.GetIsolate(), info[1], src);
  if (!src) {
  ...
  if (!info[2]->IsUndefined()) {
    src_offset = NativeValueTraits<IDLUnsignedLongLongEnforceRange>::NativeValue(info.GetIsolate(), info[2], exception_state);
    if (exception_state.HadException())
      return;
  } else {
    src_offset = 0;
  ...
  if (!info[3]->IsUndefined()) {
    byte_length = NativeValueTraits<IDLUnsignedLongLongEnforceRange>::NativeValue(info.GetIsolate(), info[3], exception_state);
    if (exception_state.HadException())
      return;
  } else {
    byte_length = 0;
