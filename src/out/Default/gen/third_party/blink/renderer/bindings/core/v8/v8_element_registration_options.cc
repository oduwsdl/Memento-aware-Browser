...
eternalV8ElementRegistrationOptionsKeys(v8::Isolate* isolate) {
...
void V8ElementRegistrationOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, ElementRegistrationOptions* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8ElementRegistrationOptionsKeys(isolate).data();
  ...
  if (prototype_value.IsEmpty() || prototype_value->IsUndefined()) {
    ...
    ScriptValue prototype_cpp_value = NativeValueTraits<IDLNullable<IDLObject>>::NativeValue(isolate, prototype_value, exception_state);
