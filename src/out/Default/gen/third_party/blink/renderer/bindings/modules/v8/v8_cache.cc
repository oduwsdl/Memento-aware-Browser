...
static void PutMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!response) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(1, "Response"));
    return;
  }
...
void V8Cache::MatchMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  UseCounter::Count(execution_context_for_measurement, WebFeature::kCacheStorageRead);
...
void V8Cache::AddMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ExecutionContext* execution_context_for_measurement = CurrentExecutionContext(info.GetIsolate());
  UseCounter::Count(execution_context_for_measurement, WebFeature::kCacheStorageWrite);
