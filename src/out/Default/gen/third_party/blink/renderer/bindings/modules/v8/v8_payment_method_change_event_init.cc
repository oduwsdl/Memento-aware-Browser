...
eternalV8PaymentMethodChangeEventInitKeys(v8::Isolate* isolate) {
...
void V8PaymentMethodChangeEventInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, PaymentMethodChangeEventInit* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8PaymentMethodChangeEventInitKeys(isolate).data();
  ...
  if (method_details_value.IsEmpty() || method_details_value->IsUndefined()) {
    ...
    ScriptValue method_details_cpp_value = NativeValueTraits<IDLNullable<IDLObject>>::NativeValue(isolate, method_details_value, exception_state);
    if (exception_state.HadException())
      return;
