...
eternalV8GPURasterizationStateDescriptorKeys(v8::Isolate* isolate) {
...
void V8GPURasterizationStateDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPURasterizationStateDescriptor* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8GPURasterizationStateDescriptorKeys(isolate).data();
  ...
  if (depth_bias_value.IsEmpty() || depth_bias_value->IsUndefined()) {
    ...
    int32_t depth_bias_cpp_value = NativeValueTraits<IDLLongEnforceRange>::NativeValue(isolate, depth_bias_value, exception_state);
