...
eternalV8GPURenderPipelineDescriptorKeys(v8::Isolate* isolate) {
...
void V8GPURenderPipelineDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPURenderPipelineDescriptor* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8GPURenderPipelineDescriptorKeys(isolate).data();
  ...
  if (sample_mask_value.IsEmpty() || sample_mask_value->IsUndefined()) {
    ...
    uint32_t sample_mask_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, sample_mask_value, exception_state);
    if (exception_state.HadException())
      return;
