...
static void GetTranslatedShaderSourceMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!shader) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("getTranslatedShaderSource", "WebGLDebugShaders", ExceptionMessages::ArgumentNotOfType(0, "WebGLShader")));
    return;
  }
