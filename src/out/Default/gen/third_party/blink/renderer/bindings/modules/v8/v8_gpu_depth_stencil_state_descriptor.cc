...
eternalV8GPUDepthStencilStateDescriptorKeys(v8::Isolate* isolate) {
...
void V8GPUDepthStencilStateDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPUDepthStencilStateDescriptor* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8GPUDepthStencilStateDescriptorKeys(isolate).data();
  ...
  if (stencil_read_mask_value.IsEmpty() || stencil_read_mask_value->IsUndefined()) {
    ...
    uint32_t stencil_read_mask_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, stencil_read_mask_value, exception_state);
    if (exception_state.HadException())
      return;
  ...
  if (stencil_write_mask_value.IsEmpty() || stencil_write_mask_value->IsUndefined()) {
    ...
    uint32_t stencil_write_mask_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, stencil_write_mask_value, exception_state);
    if (exception_state.HadException())
      return;
