...
v8::Maybe<String> V8CreateScriptCallback::Invoke(bindings::V8ValueOrScriptWrappableAdapter callback_this_value, const String& input, const HeapVector<ScriptValue>& args) {
  ...
  for (wtf_size_t i = 0; i < args.size(); ++i) {
    argv[1 + i] = ToV8(args[i], argument_creation_context, GetIsolate());
  }
  ...
  //   an IDL value of the same type as the operation's return type.
  {
    ExceptionState exception_state(GetIsolate(),
                                   ...
                                   "invoke");
    auto native_result =
        NativeValueTraits<IDLStringOrNull>::NativeValue(
