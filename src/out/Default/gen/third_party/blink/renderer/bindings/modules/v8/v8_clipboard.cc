...
static void Read1Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
...
static void Read2Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kExecutionContext, "Clipboard", "read");
  ExceptionToRejectPromiseScope reject_promise_scope(info, exception_state);
  ...
  options = NativeValueTraits<ClipboardItemOptions>::NativeValue(info.GetIsolate(), info[0], exception_state);
  if (exception_state.HadException())
    return;

  ScriptPromise result = impl->read(script_state, options);
  V8SetReturnValue(info, result.V8Value());
}

static int ReadMethodMaxArg() {
...
static void ReadMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  switch (std::min(clipboard_v8_internal::ReadMethodMaxArg(), info.Length())) {
    case 0:
      if (true) {
