...
eternalV8CanvasContextCreationAttributesModuleKeys(v8::Isolate* isolate) {
...
void V8CanvasContextCreationAttributesModule::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, CanvasContextCreationAttributesModule* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8CanvasContextCreationAttributesModuleKeys(isolate).data();
  ...
  if (RuntimeEnabledFeatures::NewCanvas2DAPIEnabled()) {
    v8::Local<v8::Value> will_read_frequently_value;
    if (!v8Object->Get(context, keys[11].Get(isolate)).ToLocal(&will_read_frequently_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (will_read_frequently_value.IsEmpty() || will_read_frequently_value->IsUndefined()) {
      // Do nothing.
    } else {
      bool will_read_frequently_cpp_value = NativeValueTraits<IDLBoolean>::NativeValue(isolate, will_read_frequently_value, exception_state);
      if (exception_state.HadException())
        return;
      impl->setWillReadFrequently(will_read_frequently_cpp_value);
    }
  }
  ...
  if (RuntimeEnabledFeatures::WebXREnabled()) {
    ...
    if (!v8Object->Get(context, keys[12].Get(isolate)).ToLocal(&xr_compatible_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
...
bool toV8CanvasContextCreationAttributesModule(const CanvasContextCreationAttributesModule* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (RuntimeEnabledFeatures::NewCanvas2DAPIEnabled()) {
    v8::Local<v8::Value> will_read_frequently_value;
    bool will_read_frequently_has_value_or_default = false;
    if (impl->hasWillReadFrequently()) {
      will_read_frequently_value = v8::Boolean::New(isolate, impl->willReadFrequently());
      will_read_frequently_has_value_or_default = true;
    } else {
      will_read_frequently_value = v8::Boolean::New(isolate, false);
      will_read_frequently_has_value_or_default = true;
    }
    if (will_read_frequently_has_value_or_default &&
        !create_property(11, will_read_frequently_value)) {
      return false;
    }
  }
  ...
  if (RuntimeEnabledFeatures::WebXREnabled()) {
    ...
    if (xr_compatible_has_value_or_default &&
        !create_property(12, xr_compatible_value)) {
      return false;
    }
  }
