...
eternalV8SerialOptionsKeys(v8::Isolate* isolate) {
...
void V8SerialOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, SerialOptions* impl, ExceptionState& exception_state) {
  if (IsUndefinedOrNull(v8_value)) {
    exception_state.ThrowTypeError("Missing required member(s): baudrate.");
    return;
  }
  ...
  const auto* keys = eternalV8SerialOptionsKeys(isolate).data();
  ...
  if (baudrate_value.IsEmpty() || baudrate_value->IsUndefined()) {
    exception_state.ThrowTypeError("required member baudrate is undefined.");
    return;
  } else {
    uint32_t baudrate_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, baudrate_value, exception_state);
  ...
  if (buffersize_value.IsEmpty() || buffersize_value->IsUndefined()) {
    ...
    uint32_t buffersize_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, buffersize_value, exception_state);
    if (exception_state.HadException())
      return;
  ...
  if (databits_value.IsEmpty() || databits_value->IsUndefined()) {
    ...
    uint8_t databits_cpp_value = NativeValueTraits<IDLOctetEnforceRange>::NativeValue(isolate, databits_value, exception_state);
    if (exception_state.HadException())
      return;
  ...
  if (stopbits_value.IsEmpty() || stopbits_value->IsUndefined()) {
    ...
    uint8_t stopbits_cpp_value = NativeValueTraits<IDLOctetEnforceRange>::NativeValue(isolate, stopbits_value, exception_state);
    if (exception_state.HadException())
      return;
...
bool toV8SerialOptions(const SerialOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (impl->hasBaudrate()) {
    baudrate_value = v8::Integer::NewFromUnsigned(isolate, impl->baudrate());
  ...
  if (impl->hasBuffersize()) {
    buffersize_value = v8::Integer::NewFromUnsigned(isolate, impl->buffersize());
    buffersize_has_value_or_default = true;
  } else {
    buffersize_value = v8::Integer::NewFromUnsigned(isolate, 255u);
    buffersize_has_value_or_default = true;
  }
...
SerialOptions* NativeValueTraits<SerialOptions>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  SerialOptions* impl = MakeGarbageCollected<SerialOptions>();
