...
eternalV8KeyframeAnimationOptionsKeys(v8::Isolate* isolate) {
...
void V8KeyframeAnimationOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, KeyframeAnimationOptions* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8KeyframeAnimationOptionsKeys(isolate).data();
  ...
  v8::Local<v8::Value> timeline_value;
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&timeline_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (timeline_value.IsEmpty() || timeline_value->IsUndefined()) {
    // Do nothing.
  } else if (timeline_value->IsNull()) {
    impl->setTimelineToNull();
  } else {
    AnimationTimeline* timeline_cpp_value = V8AnimationTimeline::ToImplWithTypeCheck(isolate, timeline_value);
    if (!timeline_cpp_value) {
      exception_state.ThrowTypeError("member timeline is not of type AnimationTimeline.");
      return;
    }
    impl->setTimeline(timeline_cpp_value);
  }
}
...
bool toV8KeyframeAnimationOptions(const KeyframeAnimationOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Value> timeline_value;
  bool timeline_has_value_or_default = false;
  if (impl->hasTimeline()) {
    timeline_value = ToV8(impl->timeline(), creationContext, isolate);
    timeline_has_value_or_default = true;
  }
  if (timeline_has_value_or_default &&
      !create_property(1, timeline_value)) {
    return false;
  }
