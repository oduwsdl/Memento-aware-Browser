...
static void UpgradeDraggedFileSystemPermissionsMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!dom_file_system) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("upgradeDraggedFileSystemPermissions", "DevToolsHost", ExceptionMessages::ArgumentNotOfType(0, "DOMFileSystem")));
    return;
  }
