...
eternalV8ShadowRootInitKeys(v8::Isolate* isolate) {
...
void V8ShadowRootInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, ShadowRootInit* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8ShadowRootInitKeys(isolate).data();
  ...
  if (RuntimeEnabledFeatures::ManualSlottingEnabled()) {
    v8::Local<v8::Value> slot_assignment_value;
    if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&slot_assignment_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (slot_assignment_value.IsEmpty() || slot_assignment_value->IsUndefined()) {
      // Do nothing.
    } else {
      V8StringResource<> slot_assignment_cpp_value = slot_assignment_value;
      if (!slot_assignment_cpp_value.Prepare(exception_state))
        return;
      const char* const kValidValues[] = {
          "manual",
          "auto",
      };
      if (!IsValidEnum(slot_assignment_cpp_value, kValidValues, base::size(kValidValues), "SlotAssignmentMode", exception_state))
        return;
      impl->setSlotAssignment(slot_assignment_cpp_value);
    }
  }
}
...
bool toV8ShadowRootInit(const ShadowRootInit* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (RuntimeEnabledFeatures::ManualSlottingEnabled()) {
    v8::Local<v8::Value> slot_assignment_value;
    bool slot_assignment_has_value_or_default = false;
    if (impl->hasSlotAssignment()) {
      slot_assignment_value = V8String(isolate, impl->slotAssignment());
      slot_assignment_has_value_or_default = true;
    }
    if (slot_assignment_has_value_or_default &&
        !create_property(2, slot_assignment_value)) {
      return false;
    }
  }
