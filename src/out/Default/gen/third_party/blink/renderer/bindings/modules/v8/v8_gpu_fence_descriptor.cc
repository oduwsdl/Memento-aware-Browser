...
eternalV8GPUFenceDescriptorKeys(v8::Isolate* isolate) {
...
void V8GPUFenceDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPUFenceDescriptor* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8GPUFenceDescriptorKeys(isolate).data();
  ...
  if (initial_value_value.IsEmpty() || initial_value_value->IsUndefined()) {
    ...
    uint64_t initial_value_cpp_value = NativeValueTraits<IDLUnsignedLongLongEnforceRange>::NativeValue(isolate, initial_value_value, exception_state);
    if (exception_state.HadException())
      return;
