...
static void IsInputPendingMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (UNLIKELY(num_args_passed <= 0)) {
    bool result = impl->isInputPending(script_state);
    V8SetReturnValueBool(info, result);
    return;
  }
  options = V8IsInputPendingOptions::ToImplWithTypeCheck(info.GetIsolate(), info[0]);
  if (!options) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("isInputPending", "Scheduling", ExceptionMessages::ArgumentNotOfType(0, "IsInputPendingOptions")));
    return;
  }

  bool result = impl->isInputPending(script_state, options);
  V8SetReturnValueBool(info, result);
}
