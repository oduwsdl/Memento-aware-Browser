// Copyright 2019 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef THIRD_PARTY_BLINK_RENDERER_BINDINGS_MODULES_V8_V8_CANVAS_FILL_RULE_H_
#define THIRD_PARTY_BLINK_RENDERER_BINDINGS_MODULES_V8_V8_CANVAS_FILL_RULE_H_

#include "third_party/blink/renderer/bindings/core/v8/generated_code_helper.h"
#include "third_party/blink/renderer/modules/modules_export.h"
#include "third_party/blink/renderer/platform/bindings/enumeration_base.h"

namespace blink {

class MODULES_EXPORT V8CanvasFillRule final : public bindings::EnumerationBase {
 public:
  enum class Enum : enum_int_t { kNonzero, kEvenodd };

  static V8CanvasFillRule Create(v8::Isolate* isolate,
                                 v8::Local<v8::Value> value,
                                 ExceptionState& exception_state) {
    const auto& result = bindings::FindIndexInEnumStringTable(
        isolate, value, string_table_, "CanvasFillRule", exception_state);
    return result.has_value()
               ? V8CanvasFillRule(static_cast<Enum>(result.value()))
               : V8CanvasFillRule();
  }
  static base::Optional<V8CanvasFillRule> Create(const String& value) {
    const auto& result =
        bindings::FindIndexInEnumStringTable(value, string_table_);
    if (!result)
      return base::nullopt;
    return V8CanvasFillRule(static_cast<Enum>(result.value()));
  }

  constexpr V8CanvasFillRule() = default;
  explicit constexpr V8CanvasFillRule(Enum value)
      : bindings::EnumerationBase(
            static_cast<enum_int_t>(value),
            string_table_[static_cast<enum_int_t>(value)]) {}
  constexpr V8CanvasFillRule(const V8CanvasFillRule&) = default;
  constexpr V8CanvasFillRule(V8CanvasFillRule&&) = default;
  ~V8CanvasFillRule() = default;

  V8CanvasFillRule& operator=(const V8CanvasFillRule&) = default;
  V8CanvasFillRule& operator=(V8CanvasFillRule&&) = default;
  V8CanvasFillRule& operator=(const String&);

  Enum AsEnum() const { return static_cast<Enum>(GetEnumValue()); }

 private:
  static constexpr const char* const string_table_[] = {"nonzero", "evenodd"};
};

inline bool operator==(const V8CanvasFillRule& lhs,
                       V8CanvasFillRule::Enum rhs) {
  return lhs.AsEnum() == rhs;
}

inline bool operator==(V8CanvasFillRule::Enum lhs,
                       const V8CanvasFillRule& rhs) {
  return lhs == rhs.AsEnum();
}

}  // namespace blink

#endif  // THIRD_PARTY_BLINK_RENDERER_BINDINGS_MODULES_V8_V8_CANVAS_FILL_RULE_H_
