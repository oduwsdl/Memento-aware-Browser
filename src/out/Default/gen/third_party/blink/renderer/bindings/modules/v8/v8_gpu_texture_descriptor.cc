...
eternalV8GPUTextureDescriptorKeys(v8::Isolate* isolate) {
...
void V8GPUTextureDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPUTextureDescriptor* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8GPUTextureDescriptorKeys(isolate).data();
  ...
  if (size_value.IsEmpty() || size_value->IsUndefined()) {
    ...
    UnsignedLongEnforceRangeSequenceOrGPUExtent3DDict size_cpp_value;
    V8UnsignedLongEnforceRangeSequenceOrGPUExtent3DDict::ToImpl(isolate, size_value, size_cpp_value, UnionTypeConversionMode::kNotNullable, exception_state);
    if (exception_state.HadException())
      return;
  ...
  if (usage_value.IsEmpty() || usage_value->IsUndefined()) {
    ...
    uint32_t usage_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, usage_value, exception_state);
    if (exception_state.HadException())
      return;
