...
static void Set1Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ScriptPromise result = impl->set(script_state, name, value, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
...
static void Set2Method(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  CookieInit* cookie_init;
  if (!info[0]->IsNullOrUndefined() && !info[0]->IsObject()) {
    exception_state.ThrowTypeError("parameter 1 ('cookieInit') is not an object.");
    return;
  }
  cookie_init = NativeValueTraits<CookieInit>::NativeValue(info.GetIsolate(), info[0], exception_state);
  if (exception_state.HadException())
    return;

  ScriptPromise result = impl->set(script_state, cookie_init, exception_state);
  if (exception_state.HadException()) {
    return;
  }
  V8SetReturnValue(info, result.V8Value());
