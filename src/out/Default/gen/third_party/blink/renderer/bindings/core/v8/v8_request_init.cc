...
eternalV8RequestInitKeys(v8::Isolate* isolate) {
...
void V8RequestInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, RequestInit* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8RequestInitKeys(isolate).data();
  ...
  if (RuntimeEnabledFeatures::TrustTokensEnabled(executionContext)) {
    v8::Local<v8::Value> trust_token_value;
    if (!v8Object->Get(context, keys[13].Get(isolate)).ToLocal(&trust_token_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (trust_token_value.IsEmpty() || trust_token_value->IsUndefined()) {
      // Do nothing.
    } else {
      TrustToken* trust_token_cpp_value = NativeValueTraits<TrustToken>::NativeValue(isolate, trust_token_value, exception_state);
      if (exception_state.HadException())
        return;
      impl->setTrustToken(trust_token_cpp_value);
    }
  }
}
...
bool toV8RequestInit(const RequestInit* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (RuntimeEnabledFeatures::TrustTokensEnabled(executionContext)) {
    v8::Local<v8::Value> trust_token_value;
    bool trust_token_has_value_or_default = false;
    if (impl->hasTrustToken()) {
      trust_token_value = ToV8(impl->trustToken(), creationContext, isolate);
      trust_token_has_value_or_default = true;
    }
    if (trust_token_has_value_or_default &&
        !create_property(13, trust_token_value)) {
      return false;
    }
  }
