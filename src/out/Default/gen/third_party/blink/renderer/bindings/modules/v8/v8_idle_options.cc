...
eternalV8IdleOptionsKeys(v8::Isolate* isolate) {
...
void V8IdleOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, IdleOptions* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8IdleOptionsKeys(isolate).data();
  ...
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&threshold_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (threshold_value.IsEmpty() || threshold_value->IsUndefined()) {
    ...
    uint32_t threshold_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, threshold_value, exception_state);
...
bool toV8IdleOptions(const IdleOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (threshold_has_value_or_default &&
      !create_property(1, threshold_value)) {
    return false;
  }
