...
eternalV8GPUColorStateDescriptorKeys(v8::Isolate* isolate) {
...
void V8GPUColorStateDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPUColorStateDescriptor* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8GPUColorStateDescriptorKeys(isolate).data();
  ...
  if (write_mask_value.IsEmpty() || write_mask_value->IsUndefined()) {
    ...
    uint32_t write_mask_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, write_mask_value, exception_state);
    if (exception_state.HadException())
      return;
