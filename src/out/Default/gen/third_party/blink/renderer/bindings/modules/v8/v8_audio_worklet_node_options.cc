...
eternalV8AudioWorkletNodeOptionsKeys(v8::Isolate* isolate) {
...
void V8AudioWorkletNodeOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, AudioWorkletNodeOptions* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8AudioWorkletNodeOptionsKeys(isolate).data();
  ...
  if (processor_options_value.IsEmpty() || processor_options_value->IsUndefined()) {
    ...
    ScriptValue processor_options_cpp_value = NativeValueTraits<IDLObject>::NativeValue(isolate, processor_options_value, exception_state);
    if (exception_state.HadException())
      return;
