...
eternalV8PaymentDetailsModifierKeys(v8::Isolate* isolate) {
...
void V8PaymentDetailsModifier::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, PaymentDetailsModifier* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8PaymentDetailsModifierKeys(isolate).data();
  ...
  if (data_value.IsEmpty() || data_value->IsUndefined()) {
    ...
    ScriptValue data_cpp_value = NativeValueTraits<IDLObject>::NativeValue(isolate, data_value, exception_state);
    if (exception_state.HadException())
      return;
