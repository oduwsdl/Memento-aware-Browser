...
eternalV8CanMakePaymentEventInitKeys(v8::Isolate* isolate) {
...
void V8CanMakePaymentEventInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, CanMakePaymentEventInit* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8CanMakePaymentEventInitKeys(isolate).data();
  ...
  if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&payment_request_origin_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[4].Get(isolate)).ToLocal(&top_origin_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
...
bool toV8CanMakePaymentEventInit(const CanMakePaymentEventInit* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (payment_request_origin_has_value_or_default &&
      !create_property(3, payment_request_origin_value)) {
    return false;
  }
  ...
  if (top_origin_has_value_or_default &&
      !create_property(4, top_origin_value)) {
    return false;
  }
