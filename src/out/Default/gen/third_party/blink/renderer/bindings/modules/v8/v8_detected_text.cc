...
eternalV8DetectedTextKeys(v8::Isolate* isolate) {
...
void V8DetectedText::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, DetectedText* impl, ExceptionState& exception_state) {
  if (IsUndefinedOrNull(v8_value)) {
    exception_state.ThrowTypeError("Missing required member(s): boundingBox, cornerPoints, rawValue.");
    return;
  }
  ...
  const auto* keys = eternalV8DetectedTextKeys(isolate).data();
  ...
  if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&raw_value_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
...
v8::Local<v8::Value> DetectedText::ToV8Impl(v8::Local<v8::Object> creationContext, v8::Isolate* isolate) const {
  v8::Local<v8::Object> v8Object = v8::Object::New(isolate);
  if (!toV8DetectedText(this, v8Object, creationContext, isolate))
    return v8::Undefined(isolate);
  return v8Object;
}

bool toV8DetectedText(const DetectedText* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  const auto* keys = eternalV8DetectedTextKeys(isolate).data();
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  ...
  if (raw_value_has_value_or_default &&
      !create_property(2, raw_value_value)) {
    return false;
  }
...
DetectedText* NativeValueTraits<DetectedText>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  DetectedText* impl = MakeGarbageCollected<DetectedText>();
  V8DetectedText::ToImpl(isolate, value, impl, exception_state);
  return impl;
}
