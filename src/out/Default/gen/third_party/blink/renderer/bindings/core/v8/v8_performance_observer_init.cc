...
eternalV8PerformanceObserverInitKeys(v8::Isolate* isolate) {
...
void V8PerformanceObserverInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, PerformanceObserverInit* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8PerformanceObserverInitKeys(isolate).data();
  ...
  if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&entry_types_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (RuntimeEnabledFeatures::EventTimingEnabled()) {
    v8::Local<v8::Value> duration_threshold_value;
    if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&duration_threshold_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (duration_threshold_value.IsEmpty() || duration_threshold_value->IsUndefined()) {
      // Do nothing.
    } else {
      double duration_threshold_cpp_value = NativeValueTraits<IDLDouble>::NativeValue(isolate, duration_threshold_value, exception_state);
      if (exception_state.HadException())
        return;
      impl->setDurationThreshold(duration_threshold_cpp_value);
    }
  }
}
...
bool toV8PerformanceObserverInit(const PerformanceObserverInit* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (entry_types_has_value_or_default &&
      !create_property(2, entry_types_value)) {
    return false;
  }
  ...
  if (RuntimeEnabledFeatures::EventTimingEnabled()) {
    v8::Local<v8::Value> duration_threshold_value;
    bool duration_threshold_has_value_or_default = false;
    if (impl->hasDurationThreshold()) {
      duration_threshold_value = v8::Number::New(isolate, impl->durationThreshold());
      duration_threshold_has_value_or_default = true;
    }
    if (duration_threshold_has_value_or_default &&
        !create_property(1, duration_threshold_value)) {
      return false;
    }
  }
