...
eternalV8RTCRtpSendParametersKeys(v8::Isolate* isolate) {
...
void V8RTCRtpSendParameters::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, RTCRtpSendParameters* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8RTCRtpSendParametersKeys(isolate).data();
  v8::TryCatch block(isolate);
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  v8::Local<v8::Value> degradation_preference_value;
  if (!v8Object->Get(context, keys[0].Get(isolate)).ToLocal(&degradation_preference_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (degradation_preference_value.IsEmpty() || degradation_preference_value->IsUndefined()) {
    // Do nothing.
  } else {
    V8StringResource<> degradation_preference_cpp_value = degradation_preference_value;
    if (!degradation_preference_cpp_value.Prepare(exception_state))
      return;
    ...
    if (!IsValidEnum(degradation_preference_cpp_value, kValidValues, base::size(kValidValues), "RTCDegradationPreference", exception_state))
      return;
    impl->setDegradationPreference(degradation_preference_cpp_value);
  }

  v8::Local<v8::Value> encodings_value;
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&encodings_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&transaction_id_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
...
bool toV8RTCRtpSendParameters(const RTCRtpSendParameters* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Value> degradation_preference_value;
  bool degradation_preference_has_value_or_default = false;
  if (impl->hasDegradationPreference()) {
    degradation_preference_value = V8String(isolate, impl->degradationPreference());
    degradation_preference_has_value_or_default = true;
  }
  if (degradation_preference_has_value_or_default &&
      !create_property(0, degradation_preference_value)) {
    return false;
  }
  ...
  if (encodings_has_value_or_default &&
      !create_property(1, encodings_value)) {
    return false;
  }
  ...
  if (transaction_id_has_value_or_default &&
      !create_property(2, transaction_id_value)) {
    return false;
  }
