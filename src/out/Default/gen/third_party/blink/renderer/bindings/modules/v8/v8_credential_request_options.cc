...
eternalV8CredentialRequestOptionsKeys(v8::Isolate* isolate) {
...
void V8CredentialRequestOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, CredentialRequestOptions* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8CredentialRequestOptionsKeys(isolate).data();
  ...
  if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&password_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[4].Get(isolate)).ToLocal(&public_key_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[5].Get(isolate)).ToLocal(&signal_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (RuntimeEnabledFeatures::SmsReceiverEnabled()) {
    v8::Local<v8::Value> otp_value;
    if (!v8Object->Get(context, keys[2].Get(isolate)).ToLocal(&otp_value)) {
      exception_state.RethrowV8Exception(block.Exception());
      return;
    }
    if (otp_value.IsEmpty() || otp_value->IsUndefined()) {
      // Do nothing.
    } else {
      OTPCredentialRequestOptions* otp_cpp_value = NativeValueTraits<OTPCredentialRequestOptions>::NativeValue(isolate, otp_value, exception_state);
      if (exception_state.HadException())
        return;
      impl->setOtp(otp_cpp_value);
    }
  }
}
...
bool toV8CredentialRequestOptions(const CredentialRequestOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (password_has_value_or_default &&
      !create_property(3, password_value)) {
    return false;
  }
  ...
  if (public_key_has_value_or_default &&
      !create_property(4, public_key_value)) {
    return false;
  }
  ...
  if (signal_has_value_or_default &&
      !create_property(5, signal_value)) {
    return false;
  }

  if (RuntimeEnabledFeatures::SmsReceiverEnabled()) {
    v8::Local<v8::Value> otp_value;
    bool otp_has_value_or_default = false;
    if (impl->hasOtp()) {
      otp_value = ToV8(impl->otp(), creationContext, isolate);
      otp_has_value_or_default = true;
    }
    if (otp_has_value_or_default &&
        !create_property(2, otp_value)) {
      return false;
    }
  }
