...
eternalV8GPUVertexAttributeDescriptorKeys(v8::Isolate* isolate) {
...
void V8GPUVertexAttributeDescriptor::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, GPUVertexAttributeDescriptor* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8GPUVertexAttributeDescriptorKeys(isolate).data();
  ...
  if (shader_location_value.IsEmpty() || shader_location_value->IsUndefined()) {
    ...
    uint32_t shader_location_cpp_value = NativeValueTraits<IDLUnsignedLongEnforceRange>::NativeValue(isolate, shader_location_value, exception_state);
    if (exception_state.HadException())
      return;
