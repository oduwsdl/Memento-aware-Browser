...
static void GetInternalResponseURLListMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!response) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("getInternalResponseURLList", "WorkerInternals", ExceptionMessages::ArgumentNotOfType(0, "Response")));
    return;
  }
...
static void GetResourcePriorityMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!worker_global) {
    exception_state.ThrowTypeError(ExceptionMessages::ArgumentNotOfType(1, "WorkerGlobalScope"));
    return;
  }
...
void V8WorkerInternals::GetResourcePriorityMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  BLINK_BINDINGS_TRACE_EVENT("WorkerInternals.getResourcePriority");
