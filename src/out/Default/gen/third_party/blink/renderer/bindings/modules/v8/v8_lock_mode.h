...
class MODULES_EXPORT V8LockMode final : public bindings::EnumerationBase {
 public:
  ...
  static V8LockMode Create(v8::Isolate* isolate,
                           v8::Local<v8::Value> value,
                           ExceptionState& exception_state) {
    const auto& result = bindings::FindIndexInEnumStringTable(
        isolate, value, string_table_, "LockMode", exception_state);
    return result.has_value() ? V8LockMode(static_cast<Enum>(result.value()))
                              : V8LockMode();
  }
  static base::Optional<V8LockMode> Create(const String& value) {
    const auto& result =
 ...
 private:
  static constexpr const char* const string_table_[] = {"shared", "exclusive"};
};
