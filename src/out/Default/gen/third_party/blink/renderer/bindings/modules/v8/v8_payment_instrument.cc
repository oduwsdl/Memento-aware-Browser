...
eternalV8PaymentInstrumentKeys(v8::Isolate* isolate) {
...
void V8PaymentInstrument::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, PaymentInstrument* impl, ExceptionState& exception_state) {
  ...
  const auto* keys = eternalV8PaymentInstrumentKeys(isolate).data();
  ...
  if (capabilities_value.IsEmpty() || capabilities_value->IsUndefined()) {
    ...
    ScriptValue capabilities_cpp_value = NativeValueTraits<IDLObject>::NativeValue(isolate, capabilities_value, exception_state);
