// services/network/public/mojom/url_response_head.mojom-lite-for-compile.js is auto generated by mojom_bindings_generator.py, do not edit

// Copyright 2018 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
'use strict';

goog.require('mojo.internal');

goog.require('proxyResolver.mojom.ProxyServer');
goog.require('mojoBase.mojom.TimeTicks');
goog.require('network.mojom.OriginPolicy');
goog.require('network.mojom.HttpRawRequestResponseInfo');
goog.require('network.mojom.ConnectionInfo');
goog.require('network.mojom.ParsedHeaders');
goog.require('network.mojom.AuthChallengeInfo');
goog.require('network.mojom.CTPolicyCompliance');
goog.require('network.mojom.FetchResponseSource');
goog.require('network.mojom.IPEndPoint');
goog.require('url.mojom.Url');
goog.require('mojoBase.mojom.UnguessableToken');
goog.require('network.mojom.HttpResponseHeaders');
goog.require('network.mojom.FetchResponseType');
goog.require('mojoBase.mojom.Time');
goog.require('network.mojom.LoadTimingInfo');
goog.require('network.mojom.SSLInfo');






goog.provide('network.mojom.URLResponseHeadSpec');
/**
 * @const { {$:!mojo.internal.MojomType}}
 * @export
 */
network.mojom.URLResponseHeadSpec =
    { $: /** @type {!mojo.internal.MojomType} */ ({}) };




mojo.internal.Struct(
    network.mojom.URLResponseHeadSpec.$,
    'URLResponseHead',
    232,
    [
      mojo.internal.StructField(
        'requestTime', 0,
        0,
        mojoBase.mojom.TimeSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'responseTime', 8,
        0,
        mojoBase.mojom.TimeSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'headers', 16,
        0,
        network.mojom.HttpResponseHeadersSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'mimeType', 24,
        0,
        mojo.internal.String,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'charset', 32,
        0,
        mojo.internal.String,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'ctPolicyCompliance', 40,
        0,
        network.mojom.CTPolicyComplianceSpec.$,
        0,
        false /* nullable */),
      mojo.internal.StructField(
        'contentLength', 48,
        0,
        mojo.internal.Int64,
        -1,
        false /* nullable */),
      mojo.internal.StructField(
        'encodedDataLength', 56,
        0,
        mojo.internal.Int64,
        -1,
        false /* nullable */),
      mojo.internal.StructField(
        'encodedBodyLength', 64,
        0,
        mojo.internal.Int64,
        -1,
        false /* nullable */),
      mojo.internal.StructField(
        'networkAccessed', 44,
        0,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'appcacheId', 72,
        0,
        mojo.internal.Int64,
        0,
        false /* nullable */),
      mojo.internal.StructField(
        'appcacheManifestUrl', 80,
        0,
        url.mojom.UrlSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'loadTiming', 88,
        0,
        network.mojom.LoadTimingInfoSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'rawRequestResponseInfo', 96,
        0,
        network.mojom.HttpRawRequestResponseInfoSpec.$,
        null,
        true /* nullable */),
      mojo.internal.StructField(
        'wasFetchedViaSpdy', 44,
        1,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'wasAlpnNegotiated', 44,
        2,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'wasAlternateProtocolAvailable', 44,
        3,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'connectionInfo', 104,
        0,
        network.mojom.ConnectionInfoSpec.$,
        0,
        false /* nullable */),
      mojo.internal.StructField(
        'mementoInfo', 44,
        4,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'mementoDatetime', 112,
        0,
        mojo.internal.String,
        "",
        false /* nullable */),
      mojo.internal.StructField(
        'alpnNegotiatedProtocol', 120,
        0,
        mojo.internal.String,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'remoteEndpoint', 128,
        0,
        network.mojom.IPEndPointSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'wasFetchedViaCache', 44,
        5,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'proxyServer', 136,
        0,
        proxyResolver.mojom.ProxyServerSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'wasFetchedViaServiceWorker', 44,
        6,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'serviceWorkerResponseSource', 108,
        0,
        network.mojom.FetchResponseSourceSpec.$,
        network.mojom.FetchResponseSource.kUnspecified,
        false /* nullable */),
      mojo.internal.StructField(
        'wasFallbackRequiredByServiceWorker', 44,
        7,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'urlListViaServiceWorker', 144,
        0,
        mojo.internal.Array(url.mojom.UrlSpec.$, false),
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'responseType', 152,
        0,
        network.mojom.FetchResponseTypeSpec.$,
        network.mojom.FetchResponseType.kBasic,
        false /* nullable */),
      mojo.internal.StructField(
        'cacheStorageCacheName', 160,
        0,
        mojo.internal.String,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'certStatus', 156,
        0,
        mojo.internal.Uint32,
        0,
        false /* nullable */),
      mojo.internal.StructField(
        'sslInfo', 168,
        0,
        network.mojom.SSLInfoSpec.$,
        null,
        true /* nullable */),
      mojo.internal.StructField(
        'corsExposedHeaderNames', 176,
        0,
        mojo.internal.Array(mojo.internal.String, false),
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'didServiceWorkerNavigationPreload', 45,
        0,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'shouldReportCorbBlocking', 45,
        1,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'asyncRevalidationRequested', 45,
        2,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'didMimeSniff', 45,
        3,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'isSignedExchangeInnerResponse', 45,
        4,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'wasInPrefetchCache', 45,
        5,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'interceptedByPlugin', 45,
        6,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'isLegacyTlsVersion', 45,
        7,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'timingAllowPassed', 46,
        0,
        mojo.internal.Bool,
        false,
        false /* nullable */),
      mojo.internal.StructField(
        'authChallengeInfo', 184,
        0,
        network.mojom.AuthChallengeInfoSpec.$,
        null,
        true /* nullable */),
      mojo.internal.StructField(
        'requestStart', 192,
        0,
        mojoBase.mojom.TimeTicksSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'responseStart', 200,
        0,
        mojoBase.mojom.TimeTicksSpec.$,
        null,
        false /* nullable */),
      mojo.internal.StructField(
        'parsedHeaders', 208,
        0,
        network.mojom.ParsedHeadersSpec.$,
        null,
        true /* nullable */),
      mojo.internal.StructField(
        'originPolicy', 216,
        0,
        network.mojom.OriginPolicySpec.$,
        null,
        true /* nullable */),
      mojo.internal.StructField(
        'recursivePrefetchToken', 224,
        0,
        mojoBase.mojom.UnguessableTokenSpec.$,
        null,
        true /* nullable */),
    ]);



goog.provide('network.mojom.URLResponseHead');

/** @record */
network.mojom.URLResponseHead = class {
  constructor() {
    /** @type { !mojoBase.mojom.Time } */
    this.requestTime;
    /** @type { !mojoBase.mojom.Time } */
    this.responseTime;
    /** @type { !network.mojom.HttpResponseHeaders } */
    this.headers;
    /** @type { !string } */
    this.mimeType;
    /** @type { !string } */
    this.charset;
    /** @type { !network.mojom.CTPolicyCompliance } */
    this.ctPolicyCompliance;
    /** @type { !boolean } */
    this.networkAccessed;
    /** @type { !boolean } */
    this.wasFetchedViaSpdy;
    /** @type { !boolean } */
    this.wasAlpnNegotiated;
    /** @type { !boolean } */
    this.wasAlternateProtocolAvailable;
    /** @type { !boolean } */
    this.mementoInfo;
    /** @type { !boolean } */
    this.wasFetchedViaCache;
    /** @type { !boolean } */
    this.wasFetchedViaServiceWorker;
    /** @type { !boolean } */
    this.wasFallbackRequiredByServiceWorker;
    /** @type { !boolean } */
    this.didServiceWorkerNavigationPreload;
    /** @type { !boolean } */
    this.shouldReportCorbBlocking;
    /** @type { !boolean } */
    this.asyncRevalidationRequested;
    /** @type { !boolean } */
    this.didMimeSniff;
    /** @type { !boolean } */
    this.isSignedExchangeInnerResponse;
    /** @type { !boolean } */
    this.wasInPrefetchCache;
    /** @type { !boolean } */
    this.interceptedByPlugin;
    /** @type { !boolean } */
    this.isLegacyTlsVersion;
    /** @type { !boolean } */
    this.timingAllowPassed;
    /** @type { !number } */
    this.contentLength;
    /** @type { !number } */
    this.encodedDataLength;
    /** @type { !number } */
    this.encodedBodyLength;
    /** @type { !number } */
    this.appcacheId;
    /** @type { !url.mojom.Url } */
    this.appcacheManifestUrl;
    /** @type { !network.mojom.LoadTimingInfo } */
    this.loadTiming;
    /** @type { (network.mojom.HttpRawRequestResponseInfo|undefined) } */
    this.rawRequestResponseInfo;
    /** @type { !network.mojom.ConnectionInfo } */
    this.connectionInfo;
    /** @type { !network.mojom.FetchResponseSource } */
    this.serviceWorkerResponseSource;
    /** @type { !string } */
    this.mementoDatetime;
    /** @type { !string } */
    this.alpnNegotiatedProtocol;
    /** @type { !network.mojom.IPEndPoint } */
    this.remoteEndpoint;
    /** @type { !proxyResolver.mojom.ProxyServer } */
    this.proxyServer;
    /** @type { !Array<!url.mojom.Url> } */
    this.urlListViaServiceWorker;
    /** @type { !network.mojom.FetchResponseType } */
    this.responseType;
    /** @type { !number } */
    this.certStatus;
    /** @type { !string } */
    this.cacheStorageCacheName;
    /** @type { (network.mojom.SSLInfo|undefined) } */
    this.sslInfo;
    /** @type { !Array<!string> } */
    this.corsExposedHeaderNames;
    /** @type { (network.mojom.AuthChallengeInfo|undefined) } */
    this.authChallengeInfo;
    /** @type { !mojoBase.mojom.TimeTicks } */
    this.requestStart;
    /** @type { !mojoBase.mojom.TimeTicks } */
    this.responseStart;
    /** @type { (network.mojom.ParsedHeaders|undefined) } */
    this.parsedHeaders;
    /** @type { (network.mojom.OriginPolicy|undefined) } */
    this.originPolicy;
    /** @type { (mojoBase.mojom.UnguessableToken|undefined) } */
    this.recursivePrefetchToken;
  }
};

